...
// registrationPayload builds the SSAP "register" payload, embedding the stored
// client key (empty on first pairing). The manifest below is the standard LG
// permission set used by community clients (Home Assistant, etc.); the TV shows
// a one-time on-screen prompt to grant it.
func registrationPayload(clientKey string) json.RawMessage {
	payload := map[string]any{
		...
		"manifest": map[string]any{
			...
			"signed": map[string]any{
				...
				"appId":    "com.lge.test",
				"vendorId": "com.lge",
				"localizedAppNames": map[string]any{
					"":      "Ottoman",
					"en-US": "Ottoman TV Control",
				},
				"localizedVendorNames": map[string]any{"": "LG Electronics"},
				"permissions": []string{
			...
			"permissions": []string{
				...
				"CONTROL_USER_INFO", "CONTROL_DISPLAY", "CONTROL_INPUT_TEXT",
				"WRITE_SETTINGS",
			},
